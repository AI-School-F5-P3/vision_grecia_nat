# requirements.txt -----------------es el segundo, también corre pero me indica desconocido.
""" 
tensorflow>=2.0.0
mtcnn==0.1.1
keras-facenet==0.3.2
...
class PretrainedFaceRecognition:
    ...
    def find_matches(self, embedding, known_embeddings, known_names):
        """Encuentra coincidencias usando distancia euclidiana con verificación adicional"""
        matches = []
        ...
        for idx, known_embedding in enumerate(known_embeddings):
            ...
            # Guardamos la distancia más pequeña
            if distance < min_distance:
                min_distance = distance
                best_match = (known_names[idx], similarity)
            
            # Solo consideramos coincidencias si superan el umbral
            if similarity > self.similarity_threshold:
                matches.append((known_names[idx], similarity))
        
        # Si hay múltiples coincidencias, verificamos que sean consistentes
        if len(matches) > 1:
            names = [m[0] for m in matches]
            # Si hay diferentes nombres con alta similitud, es probable que sea un falso positivo
            if len(set(names)) > 1:
                return []
    ...
    def process_frame(self, frame, known_embeddings, known_names):
        """Procesa cada frame con verificaciones adicionales"""
        ...
        for face_data in faces:
            ...
            if confidence < 0.95:  # Solo procesamos detecciones de alta confianza
                continue
                
            # Verificar tamaño mínimo del rostro
            x, y, w, h = face_data['box']
            if w < 100 or h < 100:  # Ignoramos rostros muy pequeños
                continue
